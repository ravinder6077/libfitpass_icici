...
//  libfitpass
//
//  Created by Ravinder Singh on 02/09/22.
//
...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"MyLoader needs to be accessed on the main thread.");
...
@interface MyLoaderRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MyLoader *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MyLoader *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MyLoader *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MyLoader *hud = (MyLoader *)subview;
...
- (void)commonInit {
    ...
    _animationType = MyLoaderAnimationFade;
    _mode = MyLoaderModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MyLoaderAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MyLoaderAnimationZoom) {
        type = animatingIn ? MyLoaderAnimationZoomIn : MyLoaderAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MyLoaderAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MyLoaderAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MyLoaderAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MyLoaderAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    MyLoaderCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MyLoaderDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = MyLoaderBackgroundStyleSolidColor;
...
//    backgroundView.backgroundColor = [UIColor whiteColor];
    NSBundle *bundle = [NSBundle bundleWithIdentifier:@"com.fitpass.libfitpass"];
    NSURL *gifURL = [bundle URLForResource:@"loader_gif" withExtension:@"gif"];
    UIImage*gifImage = [UIImage animatedImageWithAnimatedGIFData:[NSData dataWithContentsOfURL:gifURL]];
//    imgLoader.image = gifImage;
    float htttt = 70;
    float wtttt = 70;
//    UIImageView *imgView = [[UIImageView alloc] initWithFrame:self.bounds];
    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake((iOSDeviceWidth - wtttt)/2, (iOSDeviceHeight - htttt)/2, wtttt, htttt)];
    imgView.image = gifImage;
    imgView.backgroundColor = [UIColor whiteColor];
    imgView.contentMode = UIViewContentModeScaleToFill;
    imgView.layer.cornerRadius = htttt/2;
    ...
    UIButton *button = [MyLoaderRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    MyLoaderMode mode = self.mode;
    if (mode == MyLoaderModeIndeterminate) {
    ...
    else if (mode == MyLoaderModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MyLoaderModeDeterminate || mode == MyLoaderModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == MyLoaderModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == MyLoaderModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MyLoaderModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MyLoader class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MyLoader class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[MyLoader class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MyLoader class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[MyLoader class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MyLoader class]]];
#endif
...
- (void)setMode:(MyLoaderMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MyLoaderModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = MyLoaderBackgroundStyleBlur;
            ...
            _style = MyLoaderBackgroundStyleSolidColor;
...
- (void)setStyle:(MyLoaderBackgroundStyle)style {
    if (style == MyLoaderBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = MyLoaderBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    MyLoaderBackgroundStyle style = self.style;
    if (style == MyLoaderBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MyLoaderBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation MyLoader (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [MyLoader allHUDsForView:view];
    for (MyLoader *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable MyLoaderCompletionBlock)completion {
...
- (BOOL)dimBackground {
    ...
    return backgroundView.style == MyLoaderBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = MyLoaderBackgroundStyleSolidColor;
...
@implementation MyLoaderRoundedButton
