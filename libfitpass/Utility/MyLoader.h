...
@protocol MyLoaderDelegate;


extern CGFloat const MBProgressMaxOffset;

typedef NS_ENUM(NSInteger, MyLoaderMode) {
    /// UIActivityIndicatorView.
    MyLoaderModeIndeterminate,
    /// A round, pie-chart like, progress view.
    MyLoaderModeDeterminate,
    /// Horizontal progress bar.
    MyLoaderModeDeterminateHorizontalBar,
    /// Ring-shaped progress view.
    MyLoaderModeAnnularDeterminate,
    /// Shows a custom view.
    MyLoaderModeCustomView,
    /// Shows only labels.
    MyLoaderModeText
};

typedef NS_ENUM(NSInteger, MyLoaderAnimation) {
    /// Opacity animation
    MyLoaderAnimationFade,
    /// Opacity + scale animation (zoom in when appearing zoom out when disappearing)
    MyLoaderAnimationZoom,
    /// Opacity + scale animation (zoom out style)
    MyLoaderAnimationZoomOut,
    /// Opacity + scale animation (zoom in style)
    MyLoaderAnimationZoomIn
};

typedef NS_ENUM(NSInteger, MyLoaderBackgroundStyle) {
    /// Solid color background
    MyLoaderBackgroundStyleSolidColor,
    /// UIVisualEffectView or UIToolbar.layer background view
    MyLoaderBackgroundStyleBlur
};

typedef void (^MyLoaderCompletionBlock)();
 ...
 * The MyLoader window spans over the entire space given to it by the initWithFrame: constructor and catches all
 * user input on this region, thereby preventing the user operations on components below the view.
 *
 * @note To still allow touches to pass through the HUD, you can set hud.userInteractionEnabled = NO.
 * @attention MyLoader is a UI class and should therefore only be accessed on the main thread.
 */
@interface MyLoader : UIView
 ...
 */
+ (nullable MyLoader *)HUDForView:(UIView *)view;
 ...
 */
@property (weak, nonatomic) id<MyLoaderDelegate> delegate;

/**
 * Called after the HUD is hiden.
 */
@property (copy, nullable) MyLoaderCompletionBlock completionBlock;
...
/**
 * MyLoader operation mode. The default is MyLoaderModeIndeterminate.
 */
@property (assign, nonatomic) MyLoaderMode mode;
 ...
 */
@property (assign, nonatomic) MyLoaderAnimation animationType UI_APPEARANCE_SELECTOR;
...
/**
 * The UIView (e.g., a UIImageView) to be shown when the HUD is in MyLoaderModeCustomView.
...
@protocol MyLoaderDelegate <NSObject>
 ...
 */
- (void)hudWasHidden:(MyLoader *)hud;
 ...
 * Defaults to MyLoaderBackgroundStyleBlur on iOS 7 or later and MyLoaderBackgroundStyleSolidColor otherwise.
 * @note Due to iOS 7 not supporting UIVisualEffectView, the blur effect differs slightly between iOS 7 and later versions.
 */
@property (nonatomic) MyLoaderBackgroundStyle style;
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block completionBlock:(nullable MyLoaderCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue
     completionBlock:(nullable MyLoaderCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
@property (strong, nonatomic) UIColor *activityIndicatorColor __attribute__((deprecated("Use UIAppearance to customize UIActivityIndicatorView. E.g.: [UIActivityIndicatorView appearanceWhenContainedIn:[MyLoader class], nil].color = [UIColor redColor];")));
